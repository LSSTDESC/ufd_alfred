...
def cut_isochrone_path(band1, band2, band1_err, band2_err, isochrone, mag_max = 26, radius=0.1, return_all=False):
    ...
    color_diff = np.fabs((band1 - band2) - f_isochrone(band2))
    cut_2 = (color_diff < np.sqrt(0.1**2 + band1**2 + band2**2))

     # ...and now the other
    f_isochrone = scipy.interpolate.interp1d(mag_1_rgb, mag_1_rgb - mag_2_rgb, bounds_error=False, fill_value = 999.)
    color_diff = np.fabs((band1 - band2) - f_isochrone(band1))
    cut_1 = (color_diff < np.sqrt(0.1**2 + band1**2 + band2**2))
